...
class SOCReportPDF(FPDF):
    def header(self):
        ...
        self.set_text_color(57, 212, 224) # Neon cyan
...
def generate_pdf_report(df: pd.DataFrame, author: str, time_window: str) -> bytes:
    ...
    pdf.set_text_color(230, 237, 243) # Text primary
    ...
    for i, row in top_threats.iterrows():
        title = str(row.get("title", "N/A"))[:80] + ("..." if len(str(row.get("title", ""))) > 80 else "")
    ...
    if "source" in df.columns:
        ...
        for src, count in sources.items():
            pdf.cell(50, 6, str(src), 0, 0)
            pdf.cell(0, 6, f"{count} records", 0, 1)

    # Need latin-1 to avoid unicode encoding issues in basic FPDF
